...
void readParameters(ros::NodeHandle &n)
{
    ...
    fsSettings["lcExtrinsicTranslation"] >> cv_T;
    ...
    orientation.setW( tmpExtrinsic_.w() );
    orientation.setX( tmpExtrinsic_.x() );
    orientation.setY( tmpExtrinsic_.y() );
    orientation.setZ( tmpExtrinsic_.z() );

    tf::Matrix3x3(orientation).getRPY(L_C_RX, L_C_RY, L_C_RZ);
    L_C_TX = eigen_T(0);
    L_C_TY = eigen_T(1);
    L_C_TZ = eigen_T(2);
